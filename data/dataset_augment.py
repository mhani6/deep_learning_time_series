...
class LSSTPatchTSTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            if torch.rand(1) < 0.5:
                scale = 0.9 + torch.rand(x.shape[-1]) * 0.2
...
def mixup_batch(x, y, n_classes, alpha=0.4):
    """
    Mixup: interpolate pairs of samples in the batch.
    Returns mixed x and soft labels (one-hot vectors).
    alpha controls the Beta distribution — 0.4 is a good default for TS.
    """
    batch_size = x.size(0)
    ...
    y_a = torch.zeros(batch_size, n_classes).scatter_(1, y.unsqueeze(1), 1.0)
    y_b = torch.zeros(batch_size, n_classes).scatter_(1, y[idx].unsqueeze(1), 1.0)
